...
class EDMTriplaneText2ShapeOpt:
    """Options for EDM Triplane text-to-shape inference"""
    
    def __init__(self, gpu_ids=0, seed=2023):
        self.gpu_ids = [gpu_ids] if isinstance(gpu_ids, int) else gpu_ids
        self.device = torch.device(f'cuda:{gpu_ids}' if torch.cuda.is_available() else 'cpu')
        self.seed = seed
    ...
    def init_model_args(self, ckpt_path, vq_ckpt_path, 
                       vq_cfg='configs/triplane_vae_config.yaml',
                       df_cfg='configs/edm_diffusion_config.yaml'):

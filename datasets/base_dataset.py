...
def CreateDataset(opt):
    ...
    if opt.dataset_mode == 'snet':
        ...
        train_dataset.initialize(opt, 'train', cat=opt.cat, res=opt.res)
        test_dataset.initialize(opt, 'test', cat=opt.cat, res=opt.res)
    elif opt.dataset_mode == 'softnet_sdf':
        from datasets.softnet_sdf_dataset import SoftnetSDFDataset
        train_dataset = SoftnetSDFDataset()
        test_dataset = SoftnetSDFDataset()
        train_dataset.initialize(opt, 'train', cat=opt.cat, res=opt.res)
        test_dataset.initialize(opt, 'test', cat=opt.cat, res=opt.res)
    else:

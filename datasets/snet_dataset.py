...
class ShapeNetDataset(BaseDataset):
    def initialize(self, opt, phase='train', cat='all', res=64):
        ...
        with open(f'dataset_info_files/info-softnet.json') as f:
        ...
        for c in all_cats:
            ...
            with open(f'dataset_info_files/SoftNet_filelists/{synset}_{phase}.lst') as f:
                model_list_s = []
                for l in f.readlines():
                    model_id = l.rstrip('\n')
                    
                    # path = f'{dataroot}/ShapeNet/SDF_v1_64/{synset}/{model_id}/ori_sample_grid.h5'
                    # path = f'{dataroot}/Softnet/SDF_v1/resolution_{self.res}/{synset}/{model_id}/ori_sample_grid.h5'(old softnet)
                    path = f'{dataroot}/Softnet/SDF_v1/resolution_{self.res}/{model_id}/ori_sample_grid.h5'
    ...
    def __getitem__(self, index):
        ...
        sdf = torch.Tensor(sdf).view(1, 64, 64, 64) #has all the self.res here was 64 for the poorer resolution
        # print(sdf.shape)
        # sdf = sdf[:, :64, :64, :64]
    ...
    def name(self):
        return 'SoftnetDataset' #ShapeNetSDFDataset
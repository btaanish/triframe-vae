...
class Text2ShapeDataset(BaseDataset):
    ...
    def initialize(self, opt, phase='train', cat='all', res=64):
        ...
        self.text_csv = f'{dataroot}/Softnet/01/captions.robot.csv' # try without phase captions.robot_{phase}.csv
        ...
        for d in tqdm(self.data, total=len(self.data), desc=f'readinging text data from {self.text_csv}'):
            ...
            sdf_path = f'{dataroot}/Softnet/SDF_v1/resolution_{self.res}/{model_id}/ori_sample_grid.h5'
    ...
    def __getitem__(self, index):
        ...
        sdf = torch.Tensor(sdf).view(1, 64, 64, 64) # was all self.res was 128
...
# This ShapeGlotDataset is not used unless for testing

...
# scaler = torch.cuda.amp.GradScaler()  # Scales gradients for stability (Added due to insufficient memory)

# Free up memory before training
gc.collect()
torch.cuda.empty_cache()

# Limit memory allocation (prevents full GPU usage)
torch.cuda.set_per_process_memory_fraction(0.9, device=0)  # Adjust if needed
...
#     # Gradient Accumulation (change 8 to a smaller number if needed)
...
#         optimizer.zero_grad(set_to_none=True)  # Zeroing gradients efficiently
...
#             if (iter_i + 1) % accumulation_steps == 0:  # Step optimizer after accumulation
...
#                     (iter_ip1, time.time() - iter_start_time, os.path.abspath(os.path.join(opt.logs_dir, opt.name))),
#                     'blue', attrs=['bold']
#                 )
...
if __name__ == "__main__":
    ...
    print(f"Length of training dataset: {len(train_dl.dataset)}")
    print(f"Length of testing dataset: {len(test_dl.dataset)}")

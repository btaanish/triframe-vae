...
parser.add_argument('--dset', type=str, choices=['shapenet', 'abc', 'pix3d', 'building', 'softnet'], default='softnet', help='which dataset to extract sdf')
...
def get_normalize_mesh(model_file, norm_mesh_sub_dir):
    total = 16384
    #model_file = "/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/data/Softnet/01/a24/model.obj" # DELETE THIS LINE LATER
...
def create_one_sdf(sdfcommand, res, expand_rate, sdf_file, obj_file, indx, g=0.0):
    ...
    if g > 0.0:
        command_str += " -g " + str(g)
    print("Running sdf creation command:", command_str)
    result = os.system(command_str)
    if result != 0:
        print(f"SDF generation failed with error code: {result}")
    else:
        print("command successful")    
    command_str2 = f"mv \"{indx}.dist\" \"{sdf_file}\""
    
    if os.path.exists(f"{indx}.dist"):
        os.system(command_str2)
    else:
        print(f"File {indx}.dist does not exist.")
    #command_str2 = "mv " + str(indx)+".dist " + sdf_file
    print("command move sdf file:", command_str2)
    os.system(command_str2)
...
def create_sdf_obj(sdfcommand, marching_cube_command, norm_mesh_dir, sdf_dir, obj,
    ...
    if FLAGS.dset == 'abc':
        ...
        model_id = obj.split('/')[-2] 
    elif FLAGS.dset == 'softnet':
        model_id = obj.split('/')[-2]
    print(f"Model ID: {model_id}")
    norm_mesh_dir = "/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/data/Softnet/norm_mesh_dir_v1/"
    sdf_dir = "/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/data/Softnet/SDF_v1/resolution_64/" #change according to resolution
    object_dir = "/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/data/Softnet/01"
    object_dir = os.path.join(object_dir, model_id)
    norm_mesh_sub_dir = os.path.join(norm_mesh_dir, model_id)
    #norm_mesh_sub_dir = os.path.join(object_dir, model_id)
    print(f"norm sub mesh: {norm_mesh_sub_dir}")
    sdf_sub_dir = os.path.join(sdf_dir, model_id)
    print(f"sdf sub dir: {sdf_sub_dir}")
...
def create_sdf_shapenet(sdfcommand, marching_cube_command, LIB_command,
    ...
    for catnm in all_cats:
        ...
        cat_id = cats[catnm]
        if catnm == "robot":  # Special case for robot
            cat_mesh_dir = raw_dirs["mesh_dir"]
        else:
...
#     resolution = int(res // reduce)
#     sdf_root = raw_dirs["sdf_dir"]
#     sdf_dir = os.path.join(sdf_root, f'resolution_{resolution}') # .../Softnet/SDF_v1
#     if not os.path.exists(sdf_dir): os.makedirs(sdf_dir)
...
if __name__ == "__main__":
    ...
    info_file = '/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/dataset_info_files/info-softnet.json'
    ...
    isosurface_dir = '/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/preprocess/isosurface'
    ...
    # ':/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/preprocess/isosurface/tbb/tbb2018_20180822oss/lib/intel64/gcc4.7/').strip(':')
    
    tbb_dir = '/content/drive/MyDrive/Colab Notebooks/Soft-Fusion-FYP/preprocess/isosurface/tbb/tbb2018_20180822oss/lib/intel64/gcc4.7'
    nvidia_lib = '/usr/lib64-nvidia'

    # Modify LD_LIBRARY_PATH in the script directly
    os.environ['LD_LIBRARY_PATH'] = (
    os.environ.get('LD_LIBRARY_PATH', '') + 
    f':{isosurface_dir}:{tbb_dir}:{nvidia_lib}'
    ).strip(':')

    num_sample = 65 ** 3
    bandwidth = 0.1
    res = 256 # files are too big #prev 512
    reduce = FLAGS.reduce
    expand_rate = 1.3
    iso_val = 0.003
    max_verts = 16384
